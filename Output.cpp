...
Output::Output()
{
	...
	UI.CellWidth = UI.width / NumHorizontalCells; // CellWidth = 110
	UI.CellHeight = (UI.height - UI.ToolBarHeight - UI.StatusBarHeight) / NumVerticalCells; // CellHeight = 55
...
int Output::GetCellStartX(const CellPosition& cellPos) const
{
	...
	int cellStartX = cellPos.HCell() * UI.CellWidth; // We find the horizontal index of the cell, 
	...
	return cellStartX; // this line should be changed with your implementation
}
...
int Output::GetCellStartY(const CellPosition& cellPos) const
{
	...
	int cellStartY = (cellPos.VCell() * UI.CellHeight) + UI.ToolBarHeight;

	return cellStartY; // this line should be changed with your implementation
}
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	int cellStartX = GetCellStartX(cellPos);
	int cellStartY = GetCellStartY(cellPos);
	int cellEndX = cellStartX + UI.CellWidth;	// NEW
	int cellEndY = cellStartY + UI.CellHeight;	// NEW
	...
	pWind->DrawRectangle(cellStartX, cellStartY, cellEndX, cellEndY); // ASK
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	// 1- Comparing hCell of fromCell and toCell (must be equal)
	// 2- Comparing vCell of fromCell and toCell (toCell must be < fromCell)
	// (since the vCell starts from 0 at the top and ends with 8 at the bottom)

	if ((fromCell.HCell() != toCell.HCell()) || (toCell.VCell() > fromCell.VCell()) || (fromCell.VCell() == toCell.VCell()) || fromCell.GetCellNum() == 1)
	{
		return;
	}
...
///TODO: Set pen color and width using the appropriate variables of UI_Info object (UI)
	...
	for (int setPY = toStartY + UI.CellHeight; setPY <= fromStartY; setPY += UI.CellHeight)
	{
		// setPY refers to the y-coordinate to set the pen
		// Line is drawn from x12 to x34 and setPY stays the same
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		pWind->DrawLine(x12, setPY, x34, setPY);
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if ((fromCell.HCell() != toCell.HCell()) || (toCell.VCell() <= fromCell.VCell()) || (fromCell.VCell() == toCell.VCell()) || fromCell.GetCellNum() == 99)
	{
		return;
	}
	...
	int yArr[4] = { y1, y1 + yChange, y1, y1 - yChange };
